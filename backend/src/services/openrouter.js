/**
 * LLM chat completions — OpenAI (OPENAI_API_KEY) or OpenRouter
 */
...
/**
 * Chat completion (OpenAI when OPENAI_API_KEY is set, otherwise OpenRouter)
...
async function callOpenRouter(prompt, systemPrompt = '', options = {}) {
  ...
  const model = openai
    ? (options.model || config.openaiModel)
  ...
  if (openai) {
    ...
    headers['X-Title'] = 'Arlo Meeting Assistant';
  ...
  const apiUrl = openai ? OPENAI_API_URL : OPENROUTER_API_URL;
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const label = openai ? 'OpenAI' : 'OpenRouter';
      console.error(`❌ ${label} API error:`, response.status, errorText);

      if (openai) {
        if (model !== config.openaiFallbackModel) {
          console.log('🔄 Trying OpenAI fallback model:', config.openaiFallbackModel);
          return callOpenRouter(prompt, systemPrompt, {
            ...options,
            model: config.openaiFallbackModel,
          });
        }
      } else if (model !== config.fallbackModel) {
...
/**
 * Professor / classroom: topic change, glossary, and definitions from rolling window
 */
async function analyzeTopicSegment(recentText) {
  const systemPrompt = `You are an assistant for live university lectures.
Given the RECENT spoken transcript (may be partial), respond with ONLY valid JSON:
{
  "topicChanged": boolean,
  "previousTopicSummary": ["bullet 1", "bullet 2", "bullet 3"] or [] if no clear prior topic,
  "currentTopic": "short label",
  "glossary": [{"term": "...", "definition": "..."}],
  "formulas": [{"label": "...", "expression": "..."}]
}
If the lecture is still on the same topic, set topicChanged to false and keep previousTopicSummary empty.`;

  const prompt = `Recent lecture transcript:\n\n${recentText}`;
...
/**
 * Generate multiple-choice quiz from transcript text (prior class or rolling context)
 */
async function generateQuizQuestions(transcript, count = 5) {
  const systemPrompt = `You create short multiple-choice quizzes for students.
...
correctIndex is 0-based. Exactly ${count} questions.`;

  const prompt = `Create ${count} multiple-choice questions based on this lecture material:\n\n${transcript}`;

  const response = await callOpenRouter(prompt, systemPrompt, { maxTokens: 2048 });
  const cleaned = response.replace(/^```(?:json)?\s*\n?/i, '').replace(/\n?```\s*$/i, '').trim();
  try {
    const parsed = JSON.parse(cleaned);
...
/**
 * Detect professor emphasis cues in recent text
 */
async function detectProfessorCues(recentText) {
  const systemPrompt = `You detect moments when an instructor signals importance in a live class.
Output ONLY valid JSON: { "cueDetected": boolean, "reason": "short phrase", "matchedPhrases": ["..."] }
If the last segment does not contain emphasis, cueDetected false.`;
  ...
  try {
    ...
    return { cueDetected: false, reason: '', matchedPhrases: [] };
  }
}

/**
 * Recovery / post-class: explain a bookmarked moment simply + practice item
 */
async function generateRecoverySegment(transcriptSnippet) {
  const systemPrompt = `You help students who were confused during a lecture.
Output ONLY valid JSON:
{
  "simpleExplanation": "plain-language explanation",
  "practiceProblem": "one short exercise or thought question",
  "keyTerms": ["term1", "term2"]
}`;

  const prompt = `Explain this transcript segment simply and give one practice problem:\n\n${transcriptSnippet}`;

  const response = await callOpenRouter(prompt, systemPrompt, { maxTokens: 768 });
  const cleaned = response.replace(/^```(?:json)?\s*\n?/i, '').replace(/\n?```\s*$/i, '').trim();
  try {
    return JSON.parse(cleaned);
  } catch {
    return {
      simpleExplanation: cleaned,
      practiceProblem: '',
      keyTerms: [],
    };
  }
