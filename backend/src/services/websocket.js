...
function broadcastToMeeting(meetingId, message) {
  return broadcastToMeetingFiltered(meetingId, message, () => true);
}

/**
 * Broadcast to clients in a meeting that pass an optional filter (e.g. authenticated-only).
 */
function broadcastToMeetingFiltered(meetingId, message, predicate) {
  if (!connections.has(meetingId)) {
...
function transcriptRecipientPredicate(ws) {
  if (!config.requireWsAuthForTranscript) return true;
  return !!ws.userId;
}
...
function broadcastTranscriptSegment(meetingId, segment) {
  return broadcastToMeetingFiltered(
    meetingId,
    {
