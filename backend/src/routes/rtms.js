...
router.post('/broadcast', async (req, res) => {
  ...
  if (segment?.text) {
    rollingBuffer.appendFromSegment(meetingId, segment.text);
  }
...
if (process.env.NODE_ENV !== 'production') {
  ...
  /**
   * GET /api/rtms/debug-rolling-buffer?meetingId=...
   * Inspect rolling word buffer (professor-mode AI context)
   */
  router.get('/debug-rolling-buffer', (req, res) => {
    const { meetingId } = req.query;
    if (!meetingId || typeof meetingId !== 'string') {
      return res.status(400).json({ error: 'meetingId query required' });
    }
    const text = rollingBuffer.getText(meetingId);
    res.json({
      meetingId,
      wordCount: text ? text.split(/\s+/).filter(Boolean).length : 0,
      maxWords: rollingBuffer.MAX_WORDS,
      preview: text ? `${text.substring(0, 500)}${text.length > 500 ? '…' : ''}` : '',
    });
  });
}
