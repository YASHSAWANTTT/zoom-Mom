...
module.exports = {
  ...
  /** OpenAI API (api.openai.com) — when set, chat completions use OpenAI instead of OpenRouter */
  openaiApiKey: (process.env.OPENAI_API_KEY && process.env.OPENAI_API_KEY.trim()) || null,
  openaiModel: process.env.OPENAI_MODEL || 'gpt-4o-mini',
  openaiFallbackModel: process.env.OPENAI_FALLBACK_MODEL || 'gpt-3.5-turbo',
  ...
  /**
   * When true, WebSocket clients must present a valid JWT (ws ?token=) to receive
   * transcript.segment events. Anonymous meeting-only subscriptions still connect
   * for handshake but will not receive live transcript payloads (classroom / FERPA-style tightening).
   */
  requireWsAuthForTranscript: process.env.REQUIRE_WS_AUTH_FOR_TRANSCRIPT === 'true',
