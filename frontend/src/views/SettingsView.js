...
export default function SettingsView() {
  ...
  const momentumMountedRef = useRef(false);
  ...
  useEffect(() => {
    ...
    fetch('/api/preferences', { credentials: 'include' })
      ...
      .then(prefs => {
        ...
        if (prefs?.zoomMomentum?.enabled !== undefined) {
          setMomentumEnabled(!!prefs.zoomMomentum.enabled);
          try {
            localStorage.setItem('arlo-zoom-momentum-enabled', prefs.zoomMomentum.enabled ? '1' : '0');
          } catch {
            /* ignore */
          }
        }
  ...
  useEffect(() => {
    try {
      const c = localStorage.getItem('arlo-zoom-momentum-enabled');
      if (c === '1') setMomentumEnabled(true);
      if (c === '0') setMomentumEnabled(false);
    } catch {
      /* ignore */
    }
  }, []);
  ...
  useEffect(() => {
    if (!momentumMountedRef.current) {
      momentumMountedRef.current = true;
      return;
    }
    try {
      localStorage.setItem('arlo-zoom-momentum-enabled', momentumEnabled ? '1' : '0');
    } catch {
      /* ignore */
    }
    fetch('/api/preferences', {
      ...
      headers: { 'Content-Type': 'application/json' },
      credentials: 'include',
      body: JSON.stringify({ zoomMomentum: { enabled: momentumEnabled } }),
    }).catch(() => {});
  }, [momentumEnabled]);
      ...
      <section className="settings-section">
        ...
        <Card>
          <div className="settings-card-inner">
            ...
            <div className="settings-toggle-row">
              <div className="settings-toggle-text">
                <label className="text-sans font-medium" htmlFor="zoom-momentum">
                  Zoom Momentum (professor mode)
                </label>
                <p className="text-sans text-sm text-muted">
                  Show the Momentum tab in meetings: live topics, glossary, trivia, polls, and recovery.
                </p>
              </div>
              <label className="settings-toggle">
                <input
