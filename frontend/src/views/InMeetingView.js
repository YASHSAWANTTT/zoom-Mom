...
import { useZoomMomentumPreferences } from '../features/momentum/useZoomMomentumPreferences';
...
export default function InMeetingView() {
  ...
  const { momentumEnabled } = useZoomMomentumPreferences(isAuthenticated);
  ...
  const latestSegmentTimeMs = useMemo(() => {
    if (!segments.length) return Date.now();
    const s = segments[segments.length - 1];
    const n = Number(s.tStartMs);
  ...
  return (
    <div className="in-meeting-view">
      ...
      >
        <Tabs.List className="tabs-list" data-cols={momentumEnabled ? '3' : '2'}>
          ...
          {momentumEnabled && (
            <Tabs.Tab value="momentum" className="tab-trigger">Momentum</Tabs.Tab>
          )}
        </Tabs.List>
        ...
        {momentumEnabled && (
          <Tabs.Panel value="momentum" className="in-meeting-tab-panel">
            <MomentumPanel meetingId={meetingId} latestSegmentTimeMs={latestSegmentTimeMs} />
          </Tabs.Panel>
        )}
      </Tabs.Root>
    </div>
